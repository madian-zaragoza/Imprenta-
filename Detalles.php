...
// Obtener colores únicos por valor de color
$colores = [];
$coloresVistos = [];
...
while ($row = $resColor->fetch_assoc()) {
    $colorHex = strtolower(trim($row['Color']));
    if (!in_array($colorHex, $coloresVistos)) {
        $coloresVistos[] = $colorHex;
        $colores[] = $row;
    }
}
$stmt->close();
...
@media screen and (max-width: 992px) {
    ...
    /* Reorganizar la galería de imágenes en tablet */
    .thumbnails-container {
        flex-direction: row;
        justify-content: center;
        margin-top: 15px;
