...
<body>
    ...
    <div class="container">
        ...
        <form action="EditarProducto.php?id=<?php echo $idProducto; ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="card">
               ...
               <div class="form-group">
    <label>Colores:</label>
    <p class="help-text">Seleccione los colores disponibles para este producto.</p>
    <div class="color-picker-container" id="contenedor-colores">
        <?php
        if (!empty($coloresSeleccionados)) {
            // Mostrar todos los colores existentes
            foreach ($coloresSeleccionados as $valor) {
                $valorEscapado = htmlspecialchars($valor);
                echo '
                <div class="color-item">
                    <input type="color" name="colores[]" class="color-picker" value="' . $valorEscapado . '">
                </div>';
            }
        } else {
            // No hay colores, mostrar un input vacío
            echo '
            <div class="color-item">
                <input type="color" name="colores[]" class="color-picker" value="#000000">
            </div>';
        }
        ?>
    </div>
    <button type="button" onclick="agregarColor()" class="btn btn-sm btn-primary mt-2">+ Agregar color</button>
</div>
    ...
    nuevoDiv.className = 'color-item';

    const nuevoInput = document.createElement('input');
    nuevoInput.type = 'color';
    nuevoInput.name = 'colores[]';
    nuevoInput.className = 'color-picker';
    nuevoInput.value = '#000000'; // color default
