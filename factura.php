...
$sqlDetalle = "SELECT D.*, Pr.Nombre AS Producto, C.Color AS CodigoColor, T.Tamaño 
               FROM PedidoDetalle D
               JOIN Producto Pr ON D.IDproducto = Pr.IDproducto
               LEFT JOIN Color C ON D.IDcolor = C.IDcolor
               LEFT JOIN Tamaño T ON D.IDtamaño = T.IDtamaño
               WHERE D.IDpedido = ?";
...
<body>
    ...
    ?>
        <tr>
            ...
            <td>
                <?php if (!empty($row['CodigoColor'])): ?>
                    <div class="color-box" style="background-color: <?= htmlspecialchars($row['CodigoColor']) ?>"></div>
                    <?= htmlspecialchars($row['CodigoColor']) ?>
                <?php else: ?>
                    <span>Sin color</span>
                <?php endif; ?>
            </td>
